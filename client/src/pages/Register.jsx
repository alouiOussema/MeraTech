...
export default function Register() {
  ...
  useEffect(() => {
    if (voiceStep === 'WELCOME') {
      speak("مرحبا بيك في التسجيل. شنوه اسمك الكامل؟", () => setVoiceStep('NAME'));
    } else if (voiceStep === 'PIN_PROMPT') {
      speak("باهي. توا اختار PIN متكوّن من 6 أرقام.", () => setVoiceStep('PIN'));
    } else if (voiceStep === 'CONFIRM_PIN_PROMPT') {
      speak("عاود نفس الـPIN باش نتأكدو.", () => setVoiceStep('CONFIRM_PIN'));
    }
  }, [voiceStep, speak]);
  ...
  const handleSubmit = async (e, data = formData) => {
    ...
    try {
      ...
      speak("تم التسجيل بنجاح! مرحبا بيك.", () => {
        navigate('/banque');
      });
    } catch (err) {
      console.error(err);
  ...
  const handleVoiceInput = useCallback((text, intent) => {
    ...
    if (voiceStep === 'NAME') {
      if (text.length > 2) {
        ...
        speak(`مرحبا سي ${text}.`, () => setVoiceStep('PIN_PROMPT'));
      } else {
        speak("ما سمعتش اسمك بالباهي. عاود قولي اسمك.");
      }
      return;
    }
    ...
    if (voiceStep === 'PIN') {
      ...
      if (digits.length > 0) {
        ...
        if (digits.length === 6) {
          ...
          speak("PIN مريقل.", () => setVoiceStep('CONFIRM_PIN_PROMPT'));
        } else {
          speak(`سمعت ${digits.length} أرقام. لازم 6 أرقام.`);
        }
      } else {
    ...
    if (voiceStep === 'CONFIRM_PIN') {
      ...
      if (digits.length === 6) {
        if (digits === formData.pin) {
           ...
           speak("الـPIN متطابق. لحظة نسجّلك.");
           handleSubmit(null, { ...formData, confirmPin: digits });
        } else {
           speak("الـPIN موش كيف كيف. عاود الـPIN لولاني.", () => {
             setFormData(prev => ({ ...prev, pin: '', confirmPin: '' }));
             setVoiceStep('PIN_PROMPT');
           });
        }
      } else {
        speak("عاود الـPIN، لازم 6 أرقام.");
      }
    }
